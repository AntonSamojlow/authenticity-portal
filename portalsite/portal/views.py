"""
File containing all DJANGO views
...
def _get_measurements_page(group_ids: list,
    ...
    if compatible_model is not None:
        compatible_objects = []
        for measurement in objects.distinct():
            if compatible_model.is_compatible(measurement):
                compatible_objects.append(measurement)
...
class MeasurementsView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # pylint: disable=broad-except, fixme
            ...
            # TODO: Replace this error by a generic one and write stacktrace to log
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        if 'pk' not in kwargs:
            return Result(False, "Internal error",
                          details_formatted="argument 'pk' missing, model can not be identified").render_view()
        ...
        if not model.is_compatible(measurement):
            ...
            return Result(
                ...
                details_formatted=str("The submitted measurement is not compatible with the model")).render_view()
...
class MeasurementDetailView(DetailView):
    ...
    def _get_predict_choices(self) -> list[tuple[str, str]]:
        ...
        for model in Model.objects.all():
            if model.is_compatible(measurement):
    ...
    def get_context_data(self, **kwargs):
        ...
        if model_id == FilterForm.ALL:
            ...
            for prediction in predictions:
                if prediction.model.id == int(model_id):
...
class ModelDetailView(DetailView):
    ...
    def _get_trainable_measurements(self, group_id: str = FilterForm.ALL) -> 'QuerySet':
        ...
        for measurement in filtered_measurements:
            if measurement.is_labelled and model.is_compatible(measurement):
                trainable_ids.append(measurement.id)
    ...
    def _post_train(self, request: HttpRequest) -> HttpResponse:
        ...
        if not name and not request.user.has_perm('portal.change_model'):
            return Result(False, "Not allowed",
                          "You do not have the rights to change existing models").render_view()
