...
class DistanceLimits:
    ...
    def generate(
        ...
        if parameters.limit_type == LimitType.DDROBUST or parameters.limit_type == LimitType.DDMOMENTS:
            distance_limits._init_datadriven_limits()
        else:
            raise NotImplementedError("limit type not supported (during init of distance limits")
        
        return distance_limits

    def _init_datadriven_limits(self):
